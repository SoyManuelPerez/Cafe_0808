...
def generar_factura_pdf(venta, items):
    ...
    table_data = [["Producto", "Cant.", "Precio Unitario", "Subtotal"]]
    for item in items:
        table_data.append([
    ...
    prod_table = Table(table_data, colWidths=[3.5*inch, 0.8*inch, 1.35*inch, 1.35*inch])
    ...
    elements.append(Table(total_data, colWidths=[3.5*inch, 1.8*inch, 1.7*inch]))
